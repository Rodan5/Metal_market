...
for unit in units:
    ...
    with open('coins_metalmarkettest.csv', mode='a') as coins_file:
        coins_writer = csv.writer(coins_file, delimiter=';', quotechar='"', quoting=csv.QUOTE_MINIMAL)

        coins_writer.writerow([name.contents[0], price.contents[0]])

    #print("name", name.contents[0], "price", price.contents[0])